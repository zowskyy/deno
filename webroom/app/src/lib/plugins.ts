...
/** Installed plugin on this instance. */
...
/** Install a marketplace plugin on this instance. */
export function installMarketplacePlugin(slug: string): InstalledPlugin {
  const db = getDb();
  const row = db
  ...
  const now = new Date().toISOString();
  db.prepare(
    `INSERT INTO installed_plugins (id, slug, manifest_json, installed_at)
     VALUES (?, ?, ?, ?)
     ON CONFLICT(slug) DO UPDATE SET manifest_json = excluded.manifest_json`,
  ).run(id, slug, row.manifest_json, now);

  return { id, slug, manifest, installedAt: now };
}

/** List plugins installed on this instance. */
export function listInstalledPlugins(): InstalledPlugin[] {
  const db = getDb();
  const rows = db
    .prepare("SELECT id, slug, manifest_json, installed_at FROM installed_plugins ORDER BY installed_at")
